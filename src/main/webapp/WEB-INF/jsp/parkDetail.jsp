...
<p id="park-text">Founded in <span id="year-founded">${park.yearFounded}</span>, <span id="park-name">${park.parkName}</span>
is located in the state of <span id="state">${park.state}</span> and has <span id="mile-of-trail">${park.mileOfTrail}</span>
miles of trails spread out in its <span id="acreage">${park.acreage}</span> acres of natural
landscapes. With a typical <span id="climate">${park.climate}</span> climate and an elevation
of <span id="elevation">${park.elevation}</span> feet, <span id="number-of-animals">${park.numberOfAnimalSpecies}</span> different
species of animals can be spotted in the park. This park attracts
around <span id="anuual-visitors">${park.annualVisitors}</span> visitors every year, who can choose
from <span id="number-of-campsites">${park.numberOfCampSites} different camp sites.</p>
</div>
